...
Candidate evidence on 2026-08-28: 193/193 local contract tests, TypeScript, ESLint with zero warnings, the Next.js production build, and the high-severity runtime dependency audit passed. The repeat-event workflow now carries forward only the reusable intended audience while resetting accessibility review, accessibility notes, and communication intent for explicit occurrence-level approval, then routes the organizer directly to the new draft's editor. Under the controlled beta's one-active-event limit, finishing the source event and creating the next draft is now an explicit atomic transition: the source archive rolls back if any part of the repeat operation fails. Archived history is labeled accurately and cannot be directly edited or published back into an active state. New drafts retain a privacy-safe, same-owner previous-event reference so the organizer can verify lineage later; deletion clears the reference instead of deleting the newer event. Dashboard publishing now funnels through the shared guarded publication API; the former direct status update that could bypass event-brief readiness has been removed, and actionable blockers are shown to the organizer. The launch decision remains `NO_GO` at 5/18 because the 13 provider, compliance, real-host, real-device, and human-approval gates remain pending. The changed authenticated wizard and repeat transition still require authorized mobile/tablet/desktop visual QA and a disposable authenticated lifecycle after deployment; no production deployment is claimed by this checkpoint.
