...
export default async function EventDetailPage({ params, searchParams }: EventDetailPageProps) {
  // Optimized: Resolve params, searchParams, and user in parallel to reduce waterfall delay.
  ...
  // Optimized: Combined event fetch and counts into a single query with subqueries.
  // This reduces database round-trips from 3 to 1, improving TTFB.
  const eventData = await queryOne<Event & { response_count: number; guest_count: number }>(
    `SELECT e.*,
  ...
  const responseCount = eventData.response_count;
