...
export function SendAnnouncementModal({ open, onClose, eventId, onSuccess }: Props) {
  ...
  async function previewAudience() {
    ...
    const sendAt = scheduledAt ? new Date(scheduledAt).toISOString() : null;
    const response = await fetch(`/api/events/${eventId}/announcements/audience`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ subject: subject.trim(), message: message.trim(), audience, channels, sendAt }) });
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="presentation">
    <div role="dialog" aria-modal="true" aria-labelledby="announcement-title" className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-white shadow-2xl">
      ...
      <form onSubmit={submit} className="space-y-5">
        ...
        <div><label htmlFor="ann-subject" className="text-sm font-medium">Subject</label><input id="ann-subject" required maxLength={200} value={subject} onChange={(e) => { setSubject(e.target.value); setPreview(null); setApproved(false); }} className="mt-1 h-11 w-full rounded-lg border px-3" /></div>
        <div><label htmlFor="ann-message" className="text-sm font-medium">Message</label><textarea id="ann-message" required maxLength={5000} rows={5} value={message} onChange={(e) => { setMessage(e.target.value); setPreview(null); setApproved(false); }} className="mt-1 w-full rounded-lg border px-3 py-2" /></div>
