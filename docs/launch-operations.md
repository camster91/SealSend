...
The acceptance thresholds in `config/beta-acceptance-policy.json` must be approved before the first host consents. Approval requires a named approver, an ISO timestamp, all five required segments, and explicit thresholds for cohort size, workflow and response coverage, repeat use, feedback, time-to-publish, operator-recorded support time, willingness to pay, and zero unresolved severity-1 or severity-2 defects. Cameron Ashley approved `recurring-community-v1` at `2026-08-28T20:06:43.844Z`, before any host consent was recorded. The named `cohortVersion` binds invitations, consent, reporting, and aggregate metrics. Database constraints allow one active host per required segment in that cohort and one open invitation per segment. Rows migrated from earlier releases are labeled `legacy-unassigned` and are excluded from current-cohort evidence. Thresholds must not be changed after beta evidence exists; changing them requires discarding the affected cohort from launch evidence and beginning a new consented cohort under a newly approved policy. The operations metrics endpoint returns `POLICY_NOT_APPROVED`, `INVALID_POLICY`, `BETA_ACCEPTANCE_FAIL`, or `BETA_ACCEPTANCE_PASS` and preserves missing evidence as `missing`, never zero or a pass. A beta pass is evidence for the five-host and launch-metrics review only; it does not override provider, legal, accessibility, billing, production, or owner-approval gates.

The approved thresholds originated in `config/beta-acceptance-policy.proposed.json`. That immutable proposal remains a shape-validation artifact, so `npm run beta:validate-proposal` still reports `activationStatus: NOT_APPROVED`; approval is recorded only in the active policy. Repository approval does not itself deploy the policy, invite a host, record consent, or open production enrollment.
...
4. Twilio acceptance requires sending credentials, a sender, `TWILIO_AUTH_TOKEN` for callback signature validation, and the exact public status-callback and incoming-message webhook configuration. Verify sent, delivered, failed, undelivered, duplicate, invalid-signature, and transient-database retry behavior. With Advanced Opt-Out enabled, verify signed incoming STOP, START, and HELP events: STOP must add the organizer-scoped hashed suppression, START must remove it after Twilio unblocks the sender, HELP must not alter suppression state, and replay must be idempotent. The handler returns empty TwiML because Twilio has already sent the configured opt-out reply.
